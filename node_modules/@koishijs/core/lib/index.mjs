...
var Commander = class {
  ...
  available(session) {
    return this._commandList.filter((cmd) => cmd.match(session)).flatMap((cmd) => Object.entries(cmd._aliases).filter(([, alias]) => session.resolve(alias.filter) ?? true).map(([name]) => name));
  }
  resolve(key, session) {
