...
export class HTTP extends Service {
  ...
  constructor(...args: any[]) {
    let ctx: Context, config: HTTP.Config | undefined
    if (Context.is<Context>(args[0])) {
      ctx = args[0]
      config = args[1]
    } else {
      ctx = new Context()
  ...
  resolveConfig(init?: HTTP.RequestConfig): HTTP.RequestConfig {
    ...
    this.ctx.emit({ ctx: this.ctx } as any, 'http/config', result)
  ...
  async [Service.invoke](...args: any[]) {
    ...
    try {
      ...
      this.ctx.emit({ ctx: this.ctx } as any, 'http/fetch-init', url, init, config)
  ...
  ws(url: string | URL, init?: HTTP.Config) {
    ...
    if (WebSocket !== globalThis.WebSocket) {
      ...
      this.ctx.emit({ ctx: this.ctx } as any, 'http/websocket-init', url, options, config)
    }
