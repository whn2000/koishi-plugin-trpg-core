...
class DefaultRule {
    constructor() {
        this.id = 'k-trpg-default';
        this.name = 'Standard 1d100';
    }
    resolveDice(expression) {
    ...
    applyCheck(checkVal, targetVal, ruleType) {
        ...
        if (checkVal <= targetVal) {
            outcome = 'success';
            if (checkVal <= Math.floor(targetVal / 2))
                outcome = 'hard_success';
            if (checkVal <= Math.floor(targetVal / 5))
            ...
            if (checkVal > 95)
                outcome = 'fumble'; // Simplified rule
